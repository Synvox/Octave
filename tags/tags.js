...
riot.tag2('app', '<panel width="100vw" height="80px" class="toolbar-panel"></panel> <panel width="20vw" bottom="0" top="80px" class="menu-panel"> </panel> <panel width="80vw" bottom="0" top="80px" right="0" class="main-panel"> <cell pos="T0 L0 R0 H8" style="background: blue;"> </cell> <cell pos="T8 L0 R0 B0" class="songs"> <track each="{val, i in parent.parent.tracks}" index="{i}" data="{val}"></track> </cell> </panel>', 'html,body,div,span,applet,object,iframe,h1,h2,h3,h4,h5,h6,p,blockquote,pre,a,abbr,acronym,address,big,cite,code,del,dfn,em,img,ins,kbd,q,s,samp,small,strike,strong,sub,sup,tt,var,b,u,i,center,dl,dt,dd,ol,ul,li,fieldset,form,label,legend,table,caption,tbody,tfoot,thead,tr,th,td,article,aside,canvas,details,embed,figure,figcaption,footer,header,hgroup,menu,nav,output,ruby,section,summary,time,mark,audio,video{margin:0;padding:0;border:0;font-size:100%;font:inherit;vertical-align:baseline}article,aside,details,figcaption,figure,footer,header,hgroup,menu,nav,section{display:block}body{line-height:1}ol,ul{list-style:none}blockquote,q{quotes:none}blockquote:before,blockquote:after,q:before,q:after{content:\'\';content:none}table{border-collapse:collapse;border-spacing:0}html{background:#202020}app{font-family:sans-serif;color:#eee;display:block}app table{table-layout:fixed}app .toolbar-panel{background:linear-gradient(#ccc, #aaa)}app .menu-panel{background:#202020}app .main-panel{background:#efefef}app .main-panel .songs{overflow:auto}', '', function(opts) {
var self = this;
var os = require('os');
...
riot.tag2('track', '<div class="data center">{data.Name}</div> <div class="data right"> <span if="{active && currentTimeStr}">{currentTimeStr} / </span> {timeStr} </div> <div class="progress" if="{active}"> <div class="value"> </div> </div>', 'track{cursor:pointer;display:block;position:relative;overflow:hidden;color:#444;height:40px}track:nth-child(even){background:rgba(0,0,0,0.01)}track:hover{color:black;background:rgba(0,0,0,0.1)}track.active{color:#444;background:#eee}track .data{display:inline-block;margin:0 20px;line-height:40px}track .data.left{width:30px;text-align:right}track .data.center{min-width:400px;width:80vw}track .data.right{position:absolute;right:0;top:0}track h2{color:white;background:black;border-radius:3px;position:absolute;top:20px;left:100px;line-height:40px;font-size:25px;display:inline-block;padding:0 32px}track h3{color:white;background:black;border-radius:3px;position:absolute;top:62px;left:100px;line-height:30px;font-size:15px;display:inline-block;padding:0 32px}track .circle{width:70px;height:70px;background:#ff8000;position:absolute;top:20px;left:20px;border:none;border-radius:1000px}track .circle:active,track .circle.playing{background:#0080ff}track .circle.playing:active{background:#ff8000}track .progress{position:absolute;left:20px;bottom:0;right:20px;height:6px;background:#101010;border-radius:1000px;overflow:hidden}track .progress .value{background:#ff8000;border-radius:1000px;position:absolute;top:1px;left:1px;bottom:1px;width:100%;transform-origin:0;tranform:translateX(-100%)}', 'onclick="{select}"', function(opts) {
var self = this;
