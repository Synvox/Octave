...
riot.tag2('crumbs', '<ul if="{data}"> <li onclick="{reset}">Library</li> <li class="chevron" if="{data.artist}">›</li> <li if="{data.artist}" onclick="{quickSearch}" name="artist">{data.artist}</li> <li class="chevron" if="{data.album}">›</li> <li if="{data.album}" onclick="{quickSearch}" name="album">{data.album}</li> <li class="chevron" if="{data.name}">›</li> <li onclick="{quickSearch}" name="name">{data.name}</li> </ul> <form onsubmit="{forceSearch}" class="search"> <input type="submit" hidden> <input placeholder="Search" onkeyup="{typeSearch}" class="query"> <svg viewbox="0 0 24 24" class="exit" if="{hasQuery}" onclick="{resetSearch}"> <path d="M19,3H5A2,2 0 0,0 3,5V19A2,2 0 0,0 5,21H19A2,2 0 0,0 21,19V5A2,2 0 0,0 19,3M19,19H5V5H19V19M17,8.4L13.4,12L17,15.6L15.6,17L12,13.4L8.4,17L7,15.6L10.6,12L7,8.4L8.4,7L12,10.6L15.6,7L17,8.4Z"></path> </svg> </form>', 'body{background:#202020}crumbs{background:#202020}crumbs ul{height:40px;font-size:16px;color:#ddd;line-height:30px;position:absolute;bottom:0;left:20px;width:75vw;overflow:hidden;text-overflow:ellipsis;-webkit-user-select:none;white-space:nowrap}crumbs ul li{display:inline-block;cursor:pointer}crumbs ul li.chevron{font-size:2.5em;position:relative;top:6px;opacity:.5}crumbs ul li:hover{color:#fff}crumbs form.search{border:1px solid #111;border-radius:3px;position:absolute;right:10px;bottom:10px;height:30px;background:#303030;width:calc(25% - 22px)}crumbs form.search input.query{position:absolute;left:0;top:0;bottom:0;border:none;padding:0 12px;width:calc(100% - 30px);color:white;background-color:transparent}crumbs form.search input.query:focus{outline:none}crumbs form.search .exit{position:absolute;right:5px;top:5px;height:18px;cursor:pointer}crumbs form.search .exit path{fill:#ccc}crumbs form.search .exit:hover path{fill:#fff}crumbs form.search .exit:active path{fill:#888}', '', function(opts) {
...
self.typeSearch = function () {
  ...
  if (query.length > 5 || query.length === 0) self.search(false);
};
...
riot.tag2('toolbar', '<div class="inner"> <div class="left"> <svg viewbox="0 0 24 24" onclick="{playPrev}"> <path d="M11.5,12L20,18V6M11,18V6L2.5,12L11,18Z"></path> </svg> <svg viewbox="0 0 24 24" onclick="{playPause}"> <path if="{playing}" d="M14,19.14H18V5.14H14M6,19.14H10V5.14H6V19.14Z"></path> <path if="{!playing}" d="M8,5.14V19.14L19,12.14L8,5.14Z"></path> </svg> <svg viewbox="0 0 24 24" onclick="{playNext}"> <path d="M13,6V18L21.5,12M4,18L12.5,12L4,6V18Z"></path> </svg> </div> <div class="center"> <span class="time-left">{time.left}</span> <span class="time-right">{time.right}</span> <div id="waveform"></div> </div> <div class="right"> <input type="range" value="1" min="0" max="1" step="0.01" onchange="{changeVolume}"> <svg viewbox="0 0 24 24" onclick="{shuffle}" class="{active: shuffling}"> <path d="M14.83,13.41L13.42,14.82L16.55,17.95L14.5,20H20V14.5L17.96,16.54L14.83,13.41M14.5,4L16.54,6.04L4,18.59L5.41,20L17.96,7.46L20,9.5V4M10.59,9.17L5.41,4L4,5.41L9.17,10.58L10.59,9.17Z"></path> </svg> <svg viewbox="0 0 24 24" onclick="{repeat}" class="{active: repeating}"> <path d="M17,17H7V14L3,18L7,22V19H19V13H17M7,7H17V10L21,6L17,2V5H5V11H7V7Z"></path> </svg> </div> </div> <crumbs></crumbs>', 'toolbar{display:block;height:100px;position:fixed;left:0;right:0;top:0;background:white;box-shadow:0 0 2px rgba(0,0,0,0.3),0 0 16px rgba(0,0,0,0.1);overflow:hidden;z-index:10}toolbar .inner{position:absolute;top:50px;height:50px;left:0;right:0;overflow:hidden;z-index:10}toolbar crumbs{position:absolute;left:0;right:0;height:50px;top:0;z-index:9}toolbar .left{padding:4px 0;width:25vw;text-align:center;position:absolute;top:0;left:0;bottom:0;z-index:10;-webkit-user-select:none;box-shadow:0 0 2px rgba(0,0,0,0.3),0 0 16px rgba(0,0,0,0.1)}toolbar .left svg{width:40px}toolbar .left svg path{fill:#555}toolbar .left svg:active path{fill:#000}toolbar .center{text-align:center;position:absolute;top:0;right:25vw;left:25vw;bottom:0;z-index:9;border-left:1px solid #ccc;border-right:1px solid #ccc;background:rgba(0,0,0,0.03)}toolbar .center #waveform{position:absolute;left:0;right:0;bottom:0;top:0}toolbar .center canvas{width:100%}toolbar .center .time-left{position:absolute;left:0px top: 8px}toolbar .center .time-right{position:absolute;right:0px top: 8px}toolbar .right{width:25vw;text-align:center;position:absolute;top:0;right:0;bottom:0;z-index:10;padding:8px;box-shadow:0 0 2px rgba(0,0,0,0.3),0 0 16px rgba(0,0,0,0.1)}toolbar .right svg{width:24px;margin:4px 8px}toolbar .right svg path{fill:#333}toolbar .right svg.active{-webkit-filter:drop-shadow(0 0 2px rgba(112,79,220,0.2))}toolbar .right svg.active path{fill:#704FDC}toolbar .right input[type="range"]{position:relative;top:-8px;margin:0 8px;width:20%}', '', function(opts) {
var self = this;
self.playing = false;
...
riot.tag2('tracklist', '<track each="{val, i in renderedTracks}" index="{i}" data="{val}"></track> <yield></yield>', 'tracklist{color:#333;background:white;display:block;width:100%;white-space:nowrap;position:absolute;top:100px;left:0;right:0;bottom:0;overflow:auto}tracklist .cell{padding:8px;display:inline-block;float:left}tracklist .cell:nth-child(1){width:30vw}tracklist .cell:nth-child(2){width:20vw}', '', function(opts) {
var self = this;
var $r = $(self.root);
...
App.on('play_prev', () => {
  if (previousTracks.length > 0) App.trigger('select_track', previousTracks.pop());else App.trigger('select_track', parseInt(trackIndex) - 1);
});
...
function pickShuffled() {
  ...
  while (previousTracks.indexOf(pickedIndex) === false) {
    pickedIndex++;
    if (pixedIndex > tracks.length) pickedIndex = 0;
  }
